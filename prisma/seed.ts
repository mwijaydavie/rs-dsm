...
async function main() {
  ...
  // Seed first admin account if it doesn't exist
  const adminEmail = process.env.ADMIN_EMAIL || "admin@roadsafety.go.tz";
  ...
  if (existing > 0) {
    ...
    if (!fs.existsSync(csvPath)) {
      ...
      console.log("Skipping location import. This is non-critical.");
    } else {
